...
export class BillingService {
  ...
  async getUserSubscriptionStatus(userId: string) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { billingStatus: true, role: true, proSubscriptionId: true, email: true },
    });
    if (!user) {
    ...
    if (role === 'founder') {
      return { billingStatus: 'active' as const, plan: 'pro' as const, unlimited: true as const };
    }
    const plan = user.billingStatus === 'active' && user.proSubscriptionId ? ('pro' as const) : ('free' as const);
    return { billingStatus: user.billingStatus, plan, unlimited: false as const };
  }
  ...
  async createUpgradeSession(userId: string, dto: UpgradeDto) {
    ...
    const planAmount =
      dto.plan === BillingPlan.PRO_MONTHLY ? this.config.billing.proMonthlyAmount : this.config.billing.proYearlyAmount;
    const reference = `pro_${userId}_${Date.now()}`;
    const description = dto.plan === BillingPlan.PRO_MONTHLY ? 'Pro (monthly)' : 'Pro (yearly)';
    const response = await this.paynow.createPayment(reference, planAmount, description, user.email ?? undefined, {
      returnUrl: successUrl,
    });
    if (!response.success) {
      throw new Error(response.error ? String(response.error) : 'Paynow initialization failed');
    }
    ...
    return {
      reference,
      amount: planAmount,
      redirectUrl: response.redirectUrl ? String(response.redirectUrl) : undefined,
  ...
  async createPortalSession(_userId: string) {
    throw new NotImplementedException('Paynow does not expose billing portal sessions at this time.');
  }
  async createMarketplacePayment(options: {
    ...
    const response = await this.paynow.createPayment(
      ...
      { returnUrl: options.successUrl ?? this.config.billing.marketplaceSuccessUrl },
    );
    if (!response.success) {
      throw new Error(response.error ? String(response.error) : 'Paynow initialization failed');
    }
    ...
    return {
      ...
      pollUrl: response.pollUrl ? String(response.pollUrl) : undefined,
      redirectUrl: response.redirectUrl ? String(response.redirectUrl) : undefined,
      successUrl: options.successUrl ?? this.config.billing.marketplaceSuccessUrl,

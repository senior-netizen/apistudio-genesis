...
export const envSchema = z
  .object({
    ...
    EMERGENCY_ROTATION_ENABLED: z.coerce.boolean().default(true),
    ...
    BILLING_SETTLEMENT_ENABLED: z.coerce.boolean().default(true),
    BILLING_SETTLEMENT_LOOKBACK_HOURS: z.coerce.number().int().positive().default(24),
    BILLING_SETTLEMENT_MISMATCH_THRESHOLD: z.coerce.number().min(0).max(1).default(0.01),
    BILLING_SETTLEMENT_MAX_TRANSACTIONS: z.coerce.number().int().positive().max(1000).default(250),
    BILLING_SETTLEMENT_ALERT_WEBHOOK_URL: z.string().url().optional(),

...
export function SubscriptionSummary({ subscription, payments, onCancel, busy = false }: SubscriptionSummaryProps) {
  if (!subscription) {
    return (
      ...
      <Card className="border border-border/60 bg-background/80 p-6 shadow-inner">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button variant="outline" onClick={onCancel} disabled={busy} aria-disabled={busy}>
              Cancel subscription
            </Button>
          </div>
        </div>

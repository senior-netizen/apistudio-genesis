...
import { Badge, Button, Card, FounderBadge } from '@sdl/ui';
import {
  CalendarClock,
  CheckCircle2,
  Clock3,
...
export default function TeamSettingsPage() {
  ...
  const { user: currentUser, isOwner: isOwnerAccount } = useCurrentUser();
  ...
  const handleShareLinkSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
  ...
  const handleScheduleSession = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-muted">
                ...
                and orchestrate live review sessions without leaving Squirrel API Studio.
              </p>
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {collaboration.members.map((member) => {
                  ...
                  const isCurrentMember = Boolean(currentEmail) && normalizedEmail === currentEmail;
                  const showFounderBadge = isCurrentMember && isOwnerAccount;
                  const presenceDescriptor = `${member.presence} · ${formatRelativeTime(member.lastActiveAt)}`;
                  const presenceTooltip = isCurrentMember
                    ? `${presenceDescriptor} · You${showFounderBadge ? ' · Founder' : ''}`
                    : presenceDescriptor;
                  return (
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs ${
                            presenceStyles[member.presence] ?? ''
                          }`}

...
Use these settings to manage progressive rollout and rollback of remote AI:

- `@squirrel.vscode.ai.remoteEnabled` → global on/off switch for remote AI calls.
- `@squirrel.vscode.ai.remoteRolloutPercentage` → percentage of machines that should use remote AI (0-100).
- `@squirrel.vscode.ai.fallbackEnabled` → local deterministic fallback when remote is skipped or fails.
- `@squirrel.vscode.ai.telemetryEnabled` → logs rollout/success/failure telemetry events to the output channel.

Setting `remoteEnabled=false` or `remoteRolloutPercentage=0` provides an immediate rollback path.
...
- Additional production controls are available:
  - `@squirrel.vscode.cloudSync.dryRun` → validate config but skip writes during staged rollout
  - `@squirrel.vscode.cloudSync.maxConsecutiveFailures` + `@squirrel.vscode.cloudSync.cooldownMs` → circuit-breaker style cooldown after repeated failures
  - `@squirrel.vscode.cloudSync.telemetryEnabled` → emit local telemetry events in the output channel

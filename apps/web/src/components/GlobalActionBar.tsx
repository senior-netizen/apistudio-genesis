...
export function GlobalActionBar() {
  ...
  const handleCreateProject = async () => {
    ...
    try {
      const response = await apiFetch('/v1/projects', {
        ...
        body: JSON.stringify({ name: projectName || 'Untitled Project', source: 'global-action-bar' }),
      });
      if (!response.ok) {
  ...
  const handleSendRequest = async () => {
    ...
    try {
      const response = await apiFetch('/v1/ai/explain', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: requestPrompt, plan: requestPlan }),
      });
      if (!response.ok) {
      ...
      recordResponse({
        projectId: payload.projectId ?? 'draft',
        requestId: payload.requestId ?? 'generated',
        responseId: payload.responseId ?? 'latest',
      });
    } catch (error) {
  ...
  const handlePublishApi = async () => {
    ...
    try {
      const response = await apiFetch('/v1/hub/apis', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: apiTitle, description: requestPlan, ownerId: 'web-user' }),
      });
      if (!response.ok) {
        throw new Error(`Failed to publish API (${response.status})`);
      }
      const payload = await response.json();

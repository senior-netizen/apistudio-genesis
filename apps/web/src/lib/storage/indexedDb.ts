...
export async function loadWorkspace(): Promise<WorkspaceBundle | undefined> {
  try {
    const [workspace, history] = await Promise.all([fetchWorkspaceBundle(), fetchHistory()]);
...
export async function persistWorkspace(_bundle: WorkspaceBundle) {
  console.info('[storage] Workspace persistence skipped; using backend data only.');
}
...
export async function importWorkspace(bundle: WorkspaceBundle) {
  // TODO: backend import endpoint needed to support workspace imports

...
export interface RequestSlice {
  ...
  createBlankWorkingRequest: () => void;
...
export const createRequestSlice: StateCreator<AppState, [], [], RequestSlice> = (set, get) => ({
  ...
  createBlankWorkingRequest() {
    const placeholder = createEmptyRequest();
    set((state) => {
      state.workingRequest = placeholder;
      state.unsavedChanges = false;
      state.activeRequestId = null;
      state.activeRequestTabId = placeholder.id as string;
      if (!state.openRequestTabs.some((tab) => tab.requestId === placeholder.id)) {
        state.openRequestTabs.push({ requestId: placeholder.id as string, title: 'Untitled request', customTitle: false });
      }
    });
  },
  loadRequest(request) {
    const safeRequest = request
      ...
      : createEmptyRequest();
    set((state) => {
      state.workingRequest = safeRequest;
      state.unsavedChanges = false;
      const requestId = request ? String(request.id) : String(safeRequest.id);
      state.activeRequestTabId = request ? requestId : safeRequest.id as string;
      if (request) {
      ...
      } else if (!state.openRequestTabs.some((tab) => tab.requestId === requestId)) {
        state.openRequestTabs.push({ requestId, title: 'Untitled request', customTitle: false });
      }
    });
  ...
  updateWorkingRequest(updater) {
    set((state) => {
      const current = state.workingRequest ?? createEmptyRequest();
      state.workingRequest = updater(current);

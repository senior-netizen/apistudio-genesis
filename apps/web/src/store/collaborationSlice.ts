...
function pushActivity(
  ...
  activity: Omit<CollaborationActivity, "id" | "createdAt"> & { id?: string },
) {
...
export interface CollaborationSlice {
  ...
  updateMemberPresence: (
    memberId: string,
    presence: CollaborationMember["presence"],
  ) => void;
...
> = (set, get) => ({
      ...
      collaborationSocket.onmessage = (event) => {
        try {
          ...
          if (payload?.type === "presence") {
            ...
            set((draft) => {
              const member = draft.collaboration.members.find(
          ...
          if (payload?.type === "comment") {
            set((draft) => {
              draft.collaboration.comments.unshift(
                payload.comment as CollaborationComment,
              );
            });
          }
        } catch (err) {
          console.warn(
            "[collaboration] Failed to parse websocket payload",
            err,
          );
        }
      };
  ...
  inviteMember({ email, role, message, invitedBy }) {
    ...
    set((draft) => {
      draft.collaboration.invites.unshift({
        ...
        role,
        invitedBy:
          invitedBy ?? draft.collaboration.members[0]?.name ?? "System",
        status: "pending",
  ...
  removeMember(memberId) {
    set((draft) => {
      ...
      draft.collaboration.members = draft.collaboration.members.filter(
        (item) => item.id !== memberId,
      );
      if (member) {
        pushActivity(draft.collaboration, {
  ...
  resendInvite(inviteId) {
    set((draft) => {
      const invite = draft.collaboration.invites.find(
  ...
  revokeShareLink(linkId) {
    set((draft) => {
      const link = draft.collaboration.shareLinks.find(
        (item) => item.id === linkId,
      );
      if (!link) return;
      link.status = "revoked";
  ...
  async scheduleLiveSession({ title, hostId, scheduledAt, timezone, agenda }) {
    ...
    pushActivity(get().collaboration, {
      actor:
        get().collaboration.members.find((member) => member.id === hostId)
          ?.name ?? "System",
  ...
  startLiveSession(sessionId) {
    set((draft) => {
      const session = draft.collaboration.liveSessions.find(
        (item) => item.id === sessionId,
      );
      if (!session) return;
      session.status = "live";
      session.startedAt = new Date().toISOString();
      const host = draft.collaboration.members.find(
        (member) => member.id === session.hostId,
      );
      pushActivity(draft.collaboration, {
  ...
  async addComment({ userId, message }) {
    const optimistic: CollaborationComment = {
      ...
      userName:
        get().collaboration.members.find((member) => member.id === userId)
    ...
    try {
      ...
      set((draft) => {
        const index = draft.collaboration.comments.findIndex(
  ...
  scheduleResidencyCutover(region, cutoverAt) {
    set((draft) => {
      const entry = draft.collaboration.residency.find(

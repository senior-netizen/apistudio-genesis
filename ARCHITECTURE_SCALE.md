...
## Statelessness rules
- API nodes must treat in-process state as ephemeral. Sync sessions and presence caches now persist to Redis when available, with TODOs to complete full migration (`squirrel/backend/src/sync/sync.service.ts`, `apps/api/src/sync/service.ts`).
...
- Isolation rules:
  ...
  - Sync APIs enforce workspace membership; TODO: move all session/presence artifacts to Redis so tenants are isolated even across pods.
...
- `apps/api/src/sync/service.ts`: in-memory `InMemorySyncStorage` and `Map`-based sessions; keep as-is for compatibility but replace with Redis/Postgres-backed stores for stateless horizontal scaling.
- `squirrel/backend/src/sync/sync.service.ts`: session cache now persisted to Redis with TTL; presence trackers still in-memory with TODO to move to Redis pub/sub.
...
- File uploads and Prisma-backed records remain on Postgres; add composite index `(scope_type, scope_id, server_epoch)` and consider partitioning `sync_change`/`sync_state` tables by scope for millions of events.
